...
class XMPPNotifier(Notifier):
    ...
    def __init__(self, config):
        ...
        if not isinstance(config["xmpp_recipient"], list):
            ...
            self.xmpp_recipient = config["xmpp_recipient"]
        self.xmpp_send_test = config.get(
    ...
    def check_requirements(self):
        ...
        try:
                ...
                for recipient in self.xmpp_recipient:
                    if not cl.send(
                        xmpp.protocol.Message(
                            recipient,
                            "Kimsufi Crawler: Initial Test Message",
                        )
                    ):
                        raise Exception(
            ...
            _logger.error(
                "Cannot connect/auth to XMPP Server. "
    ...
    def notify(self, title, text, url=False):
        ...
        for recipient in self.xmpp_recipient:
            if not cl.send(
                xmpp.protocol.Message(recipient, body)
            ):
                _logger.error(
                    "Failed to send message to %s"
                    % (recipient,)
